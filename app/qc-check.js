...
const checks = [
  ...
  ,['legacy personal and unrelated department data removed', !/(กายภาพบำบัด|เวชกรรมฟื้นฟู|นายชาญวิทย์|สมณะ|กนกพรรณ|ช่างจัด|วรญา|1S9tfaQ)/.test(code + html)]
  ,['director hospital title is complete', html.includes('ผู้อำนวยการโรงพยาบาลขุนตาล') && code.includes('ผู้อำนวยการโรงพยาบาลขุนตาล')]
  ,['PIN fields have accessible visibility controls', html.includes('function togglePinVisibility(') && (html.match(/class="pin-visibility-btn"/g) || []).length >= 3 && html.includes('aria-label="แสดงรหัส PIN"')]
  ,['work field widened and division field reduced on one row', html.includes('leave-html-work-department') && html.includes('leave-html-work-division') && html.includes('48mm 16mm 31mm 11mm')]
  ,['prior-leave conditions remain on their original single line', html.includes("' คลอดบุตร ครั้งสุดท้ายตั้งแต่วันที่'") && !html.includes('leave-html-last-leave-date-row')]
  ,['bulk holiday import supports sheets CSV and images', html.includes('id="holidayImportFile"') && html.includes('xlsx.full.min.js') && html.includes('tesseract.min.js') && html.includes("openPinModal('importHolidays')") && code.includes('function importHolidayRows(')]
  ,['bulk holiday import validates and skips duplicates', code.includes("rows.length > 1000") && code.includes('existing.has(date) || seen.has(date)') && code.includes('lock.releaseLock()')]
